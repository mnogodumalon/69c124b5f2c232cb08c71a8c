...
const ENTITY_TABS = [
  ...
  { key: 'scope_1_direkte_emissionen', label: 'Scope 1 – Direkte Emissionen', pascal: 'Scope1DirekteEmissionen' },
  { key: 'scope_2_indirekte_energieemissionen', label: 'Scope 2 – Indirekte Energieemissionen', pascal: 'Scope2IndirekteEnergieemissionen' },
  { key: 'scope_3_weitere_indirekte_emissionen', label: 'Scope 3 – Weitere indirekte Emissionen', pascal: 'Scope3WeitereIndirekteEmissionen' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('konzernstruktur');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    konzernstruktur: new Set(),
    berichtsjahr: new Set(),
    emissionsfaktoren: new Set(),
    scope_1_direkte_emissionen: new Set(),
    scope_2_indirekte_energieemissionen: new Set(),
    scope_3_weitere_indirekte_emissionen: new Set(),
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'scope_1_direkte_emissionen': return (data as any).scope1DirekteEmissionen as Scope1DirekteEmissionen[] ?? [];
      case 'scope_2_indirekte_energieemissionen': return (data as any).scope2IndirekteEnergieemissionen as Scope2IndirekteEnergieemissionen[] ?? [];
      case 'scope_3_weitere_indirekte_emissionen': return (data as any).scope3WeitereIndirekteEmissionen as Scope3WeitereIndirekteEmissionen[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'scope_1_direkte_emissionen':
        ...
        break;
      case 'scope_2_indirekte_energieemissionen':
        lists.konzernstrukturList = (data as any).konzernstruktur ?? [];
        lists.berichtsjahrList = (data as any).berichtsjahr ?? [];
        lists.emissionsfaktorenList = (data as any).emissionsfaktoren ?? [];
        break;
      case 'scope_3_weitere_indirekte_emissionen':
        lists.konzernstrukturList = (data as any).konzernstruktur ?? [];
        lists.berichtsjahrList = (data as any).berichtsjahr ?? [];
        lists.emissionsfaktorenList = (data as any).emissionsfaktoren ?? [];
        break;
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'scope_1_direkte_emissionen' && fieldKey === 's1_einheit') {
      const match = (lists.konzernstrukturList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.einheit_name ?? '—';
    }
    if (entity === 'scope_1_direkte_emissionen' && fieldKey === 's1_berichtsjahr') {
      const match = (lists.berichtsjahrList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.anmerkungen_jahr ?? '—';
    }
    if (entity === 'scope_1_direkte_emissionen' && fieldKey === 's1_emissionsfaktor') {
      const match = (lists.emissionsfaktorenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.ef_bezeichnung ?? '—';
    }
    if (entity === 'scope_2_indirekte_energieemissionen' && fieldKey === 's2_einheit') {
      const match = (lists.konzernstrukturList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.einheit_name ?? '—';
    }
    if (entity === 'scope_2_indirekte_energieemissionen' && fieldKey === 's2_berichtsjahr') {
      const match = (lists.berichtsjahrList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.anmerkungen_jahr ?? '—';
    }
    if (entity === 'scope_2_indirekte_energieemissionen' && fieldKey === 's2_emissionsfaktor') {
      const match = (lists.emissionsfaktorenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.ef_bezeichnung ?? '—';
    }
    if (entity === 'scope_3_weitere_indirekte_emissionen' && fieldKey === 's3_einheit') {
      const match = (lists.konzernstrukturList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.einheit_name ?? '—';
    }
    if (entity === 'scope_3_weitere_indirekte_emissionen' && fieldKey === 's3_berichtsjahr') {
      const match = (lists.berichtsjahrList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.anmerkungen_jahr ?? '—';
    }
    if (entity === 'scope_3_weitere_indirekte_emissionen' && fieldKey === 's3_emissionsfaktor') {
      const match = (lists.emissionsfaktorenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.ef_bezeichnung ?? '—';
    }
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'scope_1_direkte_emissionen': return SCOPE1DIREKTEEMISSIONEN_FIELDS;
      case 'scope_2_indirekte_energieemissionen': return SCOPE2INDIREKTEENERGIEEMISSIONEN_FIELDS;
      case 'scope_3_weitere_indirekte_emissionen': return SCOPE3WEITEREINDIREKTEEMISSIONEN_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'scope_1_direkte_emissionen': return {
      ...
      case 'scope_2_indirekte_energieemissionen': return {
      ...
      case 'scope_3_weitere_indirekte_emissionen': return {
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'scope_1_direkte_emissionen' || dialogState?.entity === 'scope_1_direkte_emissionen') && (
        <Scope1DirekteEmissionenDialog
          open={createEntity === 'scope_1_direkte_emissionen' || dialogState?.entity === 'scope_1_direkte_emissionen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'scope_1_direkte_emissionen' ? handleUpdate : (fields: any) => handleCreate('scope_1_direkte_emissionen', fields)}
          defaultValues={dialogState?.entity === 'scope_1_direkte_emissionen' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'scope_2_indirekte_energieemissionen' || dialogState?.entity === 'scope_2_indirekte_energieemissionen') && (
        <Scope2IndirekteEnergieemissionenDialog
          open={createEntity === 'scope_2_indirekte_energieemissionen' || dialogState?.entity === 'scope_2_indirekte_energieemissionen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'scope_2_indirekte_energieemissionen' ? handleUpdate : (fields: any) => handleCreate('scope_2_indirekte_energieemissionen', fields)}
          defaultValues={dialogState?.entity === 'scope_2_indirekte_energieemissionen' ? dialogState.record?.fields : undefined}
          konzernstrukturList={(data as any).konzernstruktur ?? []}
          berichtsjahrList={(data as any).berichtsjahr ?? []}
          emissionsfaktorenList={(data as any).emissionsfaktoren ?? []}
      ...
      {(createEntity === 'scope_3_weitere_indirekte_emissionen' || dialogState?.entity === 'scope_3_weitere_indirekte_emissionen') && (
        <Scope3WeitereIndirekteEmissionenDialog
          open={createEntity === 'scope_3_weitere_indirekte_emissionen' || dialogState?.entity === 'scope_3_weitere_indirekte_emissionen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'scope_3_weitere_indirekte_emissionen' ? handleUpdate : (fields: any) => handleCreate('scope_3_weitere_indirekte_emissionen', fields)}
          defaultValues={dialogState?.entity === 'scope_3_weitere_indirekte_emissionen' ? dialogState.record?.fields : undefined}
          konzernstrukturList={(data as any).konzernstruktur ?? []}
          berichtsjahrList={(data as any).berichtsjahr ?? []}
          emissionsfaktorenList={(data as any).emissionsfaktoren ?? []}
      ...
      {viewState?.entity === 'scope_1_direkte_emissionen' && (
        <Scope1DirekteEmissionenViewDialog
          open={viewState?.entity === 'scope_1_direkte_emissionen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'scope_1_direkte_emissionen', record: r }); }}
          konzernstrukturList={(data as any).konzernstruktur ?? []}
          berichtsjahrList={(data as any).berichtsjahr ?? []}
          emissionsfaktorenList={(data as any).emissionsfaktoren ?? []}
        />
      )}
      {viewState?.entity === 'scope_2_indirekte_energieemissionen' && (
        <Scope2IndirekteEnergieemissionenViewDialog
          open={viewState?.entity === 'scope_2_indirekte_energieemissionen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'scope_2_indirekte_energieemissionen', record: r }); }}
          konzernstrukturList={(data as any).konzernstruktur ?? []}
          berichtsjahrList={(data as any).berichtsjahr ?? []}
          emissionsfaktorenList={(data as any).emissionsfaktoren ?? []}
        />
      )}
      {viewState?.entity === 'scope_3_weitere_indirekte_emissionen' && (
        <Scope3WeitereIndirekteEmissionenViewDialog
          open={viewState?.entity === 'scope_3_weitere_indirekte_emissionen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'scope_3_weitere_indirekte_emissionen', record: r }); }}
          konzernstrukturList={(data as any).konzernstruktur ?? []}
          berichtsjahrList={(data as any).berichtsjahr ?? []}
          emissionsfaktorenList={(data as any).emissionsfaktoren ?? []}
        />
