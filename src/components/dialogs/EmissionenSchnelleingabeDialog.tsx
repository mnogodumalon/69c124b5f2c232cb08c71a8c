...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/EmissionenSchnelleingabe';
...
export function EmissionenSchnelleingabeDialog({ open, onClose, onSubmit, defaultValues, recordId, konzernstrukturList, berichtsjahrList, emissionsfaktorenList, enablePhotoScan = true, enablePhotoLocation = true }: EmissionenSchnelleingabeDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<EmissionenSchnelleingabe['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'emissionen_schnelleingabe');
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'se_einheit': (
      <div key="se_einheit" className="space-y-1.5">
        ...
        <Combobox
          ...
          value={extractRecordId(fields.se_einheit)}
          onChange={id => setFields(f => ({ ...f, se_einheit: id ? createRecordUrl(APP_IDS.KONZERNSTRUKTUR, id) : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateKonzernstruktur("se_einheit", q)}
          createLabel="Neu in Konzernstruktur"
        />
      </div>
    ),
    'se_berichtsjahr': (
      <div key="se_berichtsjahr" className="space-y-1.5">
        ...
        <Combobox
          ...
          value={extractRecordId(fields.se_berichtsjahr)}
          onChange={id => setFields(f => ({ ...f, se_berichtsjahr: id ? createRecordUrl(APP_IDS.BERICHTSJAHR, id) : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateBerichtsjahr("se_berichtsjahr", q)}
          createLabel="Neu in Berichtsjahr"
        />
      </div>
    ),
    'se_scope': (
      <div key="se_scope" className="space-y-1.5">
    ...
    'se_unterkategorie': (
      <div key="se_unterkategorie" className="space-y-1.5">
    ...
    'se_aktivitaet': (
      <div key="se_aktivitaet" className="space-y-1.5">
    ...
    'se_emissionsfaktor': (
      <div key="se_emissionsfaktor" className="space-y-1.5">
        <Label htmlFor="se_emissionsfaktor">Emissionsfaktor</Label>
        <Combobox
          id="se_emissionsfaktor"
          placeholder=""
          items={emissionsfaktorenListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.ef_bezeichnung ?? r.record_id),
          }))}
          value={extractRecordId(fields.se_emissionsfaktor)}
          onChange={id => setFields(f => ({ ...f, se_emissionsfaktor: id ? createRecordUrl(APP_IDS.EMISSIONSFAKTOREN, id) : undefined }))}
          searchPlaceholder="Suchen…"
          emptyText="Kein Treffer"
          onCreateNew={(q) => openCreateEmissionsfaktoren("se_emissionsfaktor", q)}
          createLabel="Neu in Emissionsfaktoren"
        />
      </div>
    ),
    'se_aktivitaetsmenge': (
      <div key="se_aktivitaetsmenge" className="space-y-1.5">
        <Label htmlFor="se_aktivitaetsmenge">Aktivitätsmenge / Verbrauchsmenge</Label>
        <Input
          id="se_aktivitaetsmenge"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'se_aktivitaetsmenge')}
          placeholder=""
          value={fields.se_aktivitaetsmenge !== undefined ? fields.se_aktivitaetsmenge : (computedValues['se_aktivitaetsmenge'] ?? '')}
          onChange={e => setFields(f => ({ ...f, se_aktivitaetsmenge: clampNumberValue(formEnhancements, 'se_aktivitaetsmenge', e.target.value) }))}
        />
      </div>
    ),
    'se_einheit_menge': (
      <div key="se_einheit_menge" className="space-y-1.5">
    ...
    'se_co2e_menge': (
      <div key="se_co2e_menge" className="space-y-1.5">
        <Label htmlFor="se_co2e_menge">Berechnete CO2e-Menge (Tonnen)</Label>
        <Input
          id="se_co2e_menge"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'se_co2e_menge')}
          placeholder=""
          value={fields.se_co2e_menge !== undefined ? fields.se_co2e_menge : (computedValues['se_co2e_menge'] ?? '')}
          onChange={e => setFields(f => ({ ...f, se_co2e_menge: clampNumberValue(formEnhancements, 'se_co2e_menge', e.target.value) }))}
        />
      </div>
    ),
    'se_datenqualitaet': (
      <div key="se_datenqualitaet" className="space-y-1.5">
    ...
    'se_bemerkungen': (
      <div key="se_bemerkungen" className="space-y-1.5">
    ...
    'se_nachweis': (
      <div key="se_nachweis" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"se_einheit": "Organisationseinheit", "se_berichtsjahr": "Berichtsjahr", "se_scope": "Scope", "se_unterkategorie": "Unterkategorie / Scope-3-Kategorie", "se_aktivitaet": "Aktivitätsbeschreibung", "se_emissionsfaktor": "Emissionsfaktor", "se_aktivitaetsmenge": "Aktivitätsmenge / Verbrauchsmenge", "se_einheit_menge": "Einheit", "se_co2e_menge": "Berechnete CO2e-Menge (Tonnen)", "se_datenqualitaet": "Datenqualität", "se_bemerkungen": "Bemerkungen", "se_nachweis": "Nachweis / Beleg (Datei-Upload)"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"se_einheit": {"einheit_name": "Name der Einheit", "einheit_typ": "Typ der Einheit", "uebergeordnete_einheit": "Übergeordnete Einheit (Name)", "land": "Land", "branche": "Branche", "konsolidierungsmethode": "Konsolidierungsmethode", "verantwortlich_vorname": "Vorname der verantwortlichen Person", "verantwortlich_nachname": "Nachname der verantwortlichen Person", "verantwortlich_email": "E-Mail der verantwortlichen Person", "anmerkungen_einheit": "Anmerkungen"}, "se_berichtsjahr": {"jahr": "Berichtsjahr", "startdatum": "Startdatum", "enddatum": "Enddatum", "ist_basisjahr": "Ist Basisjahr", "status_jahr": "Status", "anmerkungen_jahr": "Anmerkungen zum Berichtsjahr"}, "se_emissionsfaktor": {"ef_bezeichnung": "Bezeichnung", "ef_scope": "Scope-Zuordnung", "ef_kategorie": "Kategorie", "ef_energietraeger": "Energieträger / Aktivität", "ef_einheit": "Einheit", "ef_faktor": "Emissionsfaktor (kg CO2e pro Einheit)", "ef_treibhausgas": "Treibhausgase", "ef_quelle": "Quelle / Referenz", "ef_gueltigkeitsjahr": "Gültigkeitsjahr"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.EMISSIONEN_SCHNELLEINGABE} recordId={recordId} />
              </div>
            )}
          </div>
